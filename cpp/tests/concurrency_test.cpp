// Concurrency test, meant for ThreadSanitizer (`make test SANITIZE=thread`).
//
// Exercises the documented contract that a single ProtoSchema supports
// concurrent readers (descriptor_bytes / encode_json are const). Hermetic, so
// it runs in CI. No concurrent-Stream case: a Stream is not safe for concurrent
// use, so that would test misuse.
