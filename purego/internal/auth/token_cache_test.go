...
func TestTokenCacheConcurrentMintFailureSharesError(t *testing.T) {
	...
	for i := range goroutines {
		go func(i int) {
			defer wg.Done()
			_, errs[i] = c.getOrFetch(context.Background(), "id", "secret", "c.s.t",
				func(_ context.Context, _ mintReason) (fetchedToken, error) {
					calls.Add(1)
					time.Sleep(20 * time.Millisecond) // hold the flight so others park on it
	...
	// The leader's error is shared with every parked waiter: one mint, not N.
	if n := calls.Load(); n != 1 {
