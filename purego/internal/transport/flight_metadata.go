...
// FlightStreamReadyOffset is the sentinel in the first DoPut PutResult: auth,
// table access, and schema validation all succeeded.
const FlightStreamReadyOffset int64 = -1

// FlightBatchMetadata rides in FlightData.AppMetadata. OffsetID is
// connection-local and must be sequential from zero.
type FlightBatchMetadata struct {
	OffsetID int64 `json:"offset_id"`
}

// FlightAckMetadata rides in PutResult.AppMetadata.
type FlightAckMetadata struct {
	// Highest durable frame offset. FlightStreamReadyOffset is reserved for setup.
	AckUpToOffset int64 `json:"ack_up_to_offset"`
	// Cumulative durable row count.
	AckUpToRecords uint64 `json:"ack_up_to_records"`
	// Requests connection rotation after this grace period.
	CloseStreamDurationMS *uint64 `json:"close_stream_duration_ms,omitempty"`
}

// IsStreamReady reports whether this is the setup-ready response.
...
// UnmarshalJSON is strict because a permissive zero-value decode would turn
// malformed metadata such as {} into a real offset-zero frame.
...
// UnmarshalJSON is as strict as FlightBatchMetadata.UnmarshalJSON, except that
// the close signal is optional.
...
// decodeStrictJSONObject decodes exactly one JSON object. Values stay raw so
// each field decoder can check its own numeric type. Unknown fields are ignored
// for additive compatibility; duplicate names are rejected even then.
