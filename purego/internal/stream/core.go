...
// materialization is expensive (notably Arrow serialize/compress work).
//
...
func (cs *CoreStream[Req, Resp]) receiver(
	...
	resolveAck := func(resp Resp, legacyOffset int64, state AckState) (AckResolution, error) {
		...
		var err error
		if model, ok := cs.ackMdl.(resolvingAckModel[Resp]); ok {
			resolution, err = model.resolve(resp, state)
		} else {
			resolution, err = resolveOffsetAck(legacyOffset, state)
		}
		if err != nil {
	...
	handleSendEvent := func(event sendEvent) error {
		...
		if sending == nil ||
			...
			sendConsumed <- struct{}{}
			return fmt.Errorf("stream: Send completion does not match active submission")
		}
		...
		if pendingAck {
			state := ackState(false)
			resolution, err := ResolveAcknowledgedUnits(
				pendingAckUnits, state,
			)
			if err != nil {
