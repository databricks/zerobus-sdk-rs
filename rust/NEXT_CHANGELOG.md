...
- **Arrow Flight — authorization metadata is now sensitive** (Beta): Bearer credentials are marked sensitive in tonic metadata, matching the standard gRPC stream and preventing token values from appearing in metadata debug output.
