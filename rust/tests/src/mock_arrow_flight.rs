...
pub enum MockFlightResponse {
    ...
    /// via `from_setup_status`. Unlike [`MockFlightResponse::Error`], this fires
    /// on the schema message rather than on the first data batch, so it
    /// exercises the setup error path.
