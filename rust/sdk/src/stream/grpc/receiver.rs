...
//!
//! Transport-specific: reads server messages from the `InboundStream` (which
//! hides whether the RPC is `EphemeralStream` or `PersistentStream`), dispatches
//! durability acks to oneshot senders / callbacks, and signals the supervisor
//! via `server_error_tx` / pause deadlines.
...
use crate::databricks::zerobus::{CloseStreamSignal, IngestRecordResponse};
use crate::{OffsetId, StreamConfigurationOptions, ZerobusError, ZerobusResult};

impl ZerobusStream {
    /// Spawns a task that continuously reads server messages and propagates the
    /// received durability acknowledgements to the corresponding pending acks
    /// promises.
    ///
    /// `initial_last_acked_offset` is the wire offset already acknowledged when
    /// this session starts: `-1` for a fresh stream (ephemeral, or a new
    ...
    pub(super) fn spawn_receiver_task(
        ...
        tokio::spawn(async move {
            ...
            let mut last_acked_offset = initial_last_acked_offset;
            ...
            'recv_loop: loop {
                ...
                match message_result {
                    ...
                    Ok(Ok(Some(InboundMessage::Close(CloseStreamSignal { duration })))) => {
                        if options.recovery {
                            ...
                            if wait_duration_ms == 0 {
                                info!(
                                    "Server will close the stream. Triggering immediate recovery."
                                );
                                break 'recv_loop;
                            }
                    ...
                    Ok(Ok(Some(InboundMessage::Other))) => {
