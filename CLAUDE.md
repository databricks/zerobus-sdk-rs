...
This is a polyglot SDK monorepo. A single Rust core (`rust/sdk/`) implements gRPC streaming, OAuth, recovery, and ingestion logic. Seven wrapper SDKs expose it to other languages:
...
| C++        | `cpp/`         | C FFI (`rust/ffi/`)      | CMake            |
| .NET       | `dotnet/`      | P/Invoke (`rust/ffi/`)   | MSBuild / NuGet  |

The data flow is: **User code → Wrapper API → FFI boundary → Rust core → gRPC → Zerobus service**.

Go, C++, and .NET use the C FFI layer (`rust/ffi/`, header auto-generated by cbindgen). Java has its own JNI crate (`rust/jni/`). Python and TypeScript bind Rust directly via PyO3 and NAPI-RS respectively.
...
- Changing the FFI C header (`rust/ffi/zerobus.h`) in a non-additive way breaks Go, C++, and .NET. Treat FFI signature changes with the same rigor as public API changes.
...
- .NET has no finalizers on streams — users must call `Dispose()` or use `using` statements. `SafeHandle` provides critical-finalization as a backstop.
...
- .NET: thread-safe — uses `ReaderWriterLockSlim` internally for concurrent ingest.
...
| C++        | `make build`           | `make test`     | `make lint`        | `make fmt`          |
| .NET       | `dotnet build`         | `dotnet test`   | `dotnet format --verify-no-changes` | `dotnet format`    |
...
| C++        | `cpp/include/zerobus/version.hpp` + `cpp/CMakeLists.txt` | Source / CMake (GitHub Release planned) | `cpp/v*` (planned) |
| FFI        | `rust/ffi/Cargo.toml`      | GitHub Release | `ffi/v*`          |

The FFI library has its own release cycle because Go and Java depend on pre-built static/dynamic libraries. An FFI release must happen before Go/Java can pick up Rust core changes. (C++ builds the FFI from local Rust source by default, so it needs the FFI source present rather than a pre-built library; a C++ release still bundles a built FFI archive.)
