...
| C++        | `cpp/include/zerobus/version.hpp` + `cpp/CMakeLists.txt` | Source / GitHub Release | `cpp/v*` |
| FFI        | `rust/ffi/Cargo.toml`      | GitHub Release | `ffi/v*`          |

The FFI library has its own release cycle because Go, Java, and C++ depend on pre-built static/dynamic libraries. An FFI release must happen before Go/Java/C++ can pick up Rust core changes.
