"""
Synchronous Zerobus SDK (Rust-backed).

High-performance synchronous interface for ingesting records into Databricks
tables via the Zerobus service.

Example:
    from zerobus import ZerobusSdk, OAuth, Format, StreamConfigurationOptions

    sdk = ZerobusSdk(
        host="https://<shard>.zerobus.<region>.cloud.databricks.com",
        unity_catalog_url="https://<workspace>.cloud.databricks.com",
        application_name="my-app",  # optional, appended to user-agent
    )

    # JSON stream
    stream = sdk.create_stream(
        table="catalog.schema.table",
        auth=OAuth("client-id", "client-secret"),
        record_format=Format.JSON,
    )
    offset = stream.ingest_record_offset({"k": "v"})  # dict (or str)
    stream.flush()
    stream.close()

    # Protobuf stream
    from my_proto_pb2 import MyMessage
    stream = sdk.create_stream(
        table="catalog.schema.table",
        auth=OAuth("client-id", "client-secret"),
        record_format=Format.proto(MyMessage.DESCRIPTOR),
        options=StreamConfigurationOptions(max_inflight_records=10000),
    )
"""

from typing import Any, List, Optional

import zerobus._zerobus_core as _core
from zerobus.sdk.shared.auth import Auth, Headers, OAuth
from zerobus.sdk.shared.format import (
    Format,
    FormatSpec,
    _descriptor_to_bytes,
    _Json,
    _Proto,
)

_RustZerobusSdk = _core.sync.ZerobusSdk
_RustZerobusStream = _core.sync.ZerobusStream


class ZerobusStream:
    """Synchronous gRPC ingestion stream (JSON or compiled protobuf)."""

    def __init__(self, rust_stream: _RustZerobusStream):
        self._inner = rust_stream

    def ingest_record_offset(self, payload: Any) -> int:
        """Submit a record and return the offset assigned by the server."""
        return self._inner.ingest_record_offset(payload)

    def ingest_record_nowait(self, payload: Any) -> None:
        """Submit a record without waiting for an offset (fire-and-forget)."""
        return self._inner.ingest_record_nowait(payload)

    def ingest_records_offset(self, payloads: List[Any]) -> Optional[int]:
        """Submit a batch and return one offset for the whole batch."""
        return self._inner.ingest_records_offset(payloads)

    def ingest_records_nowait(self, payloads: List[Any]) -> None:
        """Submit a batch without waiting (fire-and-forget)."""
        return self._inner.ingest_records_nowait(payloads)

    def wait_for_offset(self, offset: int) -> None:
        """Block until the given offset is acknowledged."""
        return self._inner.wait_for_offset(offset)

    def flush(self) -> None:
        """Wait for all pending records to be acknowledged."""
        return self._inner.flush()

    def close(self) -> None:
        """Close the stream gracefully."""
        return self._inner.close()

    def get_unacked_records(self) -> List[bytes]:
        """Return record payloads not yet acknowledged by the server."""
        return self._inner.get_unacked_records()

    def get_unacked_batches(self) -> List[List[bytes]]:
        """Return batches (each a list of bytes) not yet acknowledged."""
        return self._inner.get_unacked_batches()


class ZerobusArrowStream:
    """Synchronous Arrow Flight stream for ingesting pyarrow RecordBatches.

    **Beta**: Arrow Flight ingestion is in Beta. The API is stabilising but
    may still change before reaching GA.
    """

    def __init__(self, rust_stream):
        self._inner = rust_stream

    def ingest_batch(self, batch) -> int:
        """Ingest a `pyarrow.RecordBatch` or `pyarrow.Table` and return its offset."""
        from zerobus.sdk.shared.arrow import _serialize_batch

        ipc_bytes = _serialize_batch(batch)
        return self._inner.ingest_batch(ipc_bytes)

    def wait_for_offset(self, offset: int) -> None:
        return self._inner.wait_for_offset(offset)

    def flush(self) -> None:
        return self._inner.flush()

    def close(self) -> None:
        return self._inner.close()

    @property
    def is_closed(self) -> bool:
        return self._inner.is_closed

    @property
    def table_name(self) -> str:
        return self._inner.table_name

    def get_unacked_batches(self) -> list:
        """Return unacknowledged batches as a list of `pyarrow.RecordBatch`."""
        from zerobus.sdk.shared.arrow import _deserialize_batch

        ipc_list = self._inner.get_unacked_batches()
        return [_deserialize_batch(ipc_bytes) for ipc_bytes in ipc_list]


def _split_auth(auth: Auth):
    """Return `(client_id, client_secret, headers_provider)` for the binding."""
    if isinstance(auth, OAuth):
        return auth.client_id, auth.client_secret, None
    if isinstance(auth, Headers):
        return None, None, auth.provider
    raise TypeError(f"auth must be OAuth(...) or Headers(...); got {type(auth).__name__}")


def _split_format(fmt: FormatSpec):
    """Return `(descriptor_bytes, message_name, record_type_value)`."""
    if isinstance(fmt, _Json):
        return None, None, _core.RecordType.JSON
    if isinstance(fmt, _Proto):
        descriptor_bytes, name_from_descriptor = _descriptor_to_bytes(fmt.descriptor)
        # `fmt.message_name` is set by `Format.proto(...)` when it could be
        # extracted from a Descriptor object; raw-bytes input has `None`.
        message_name = fmt.message_name or name_from_descriptor
        return descriptor_bytes, message_name, _core.RecordType.PROTO
    raise TypeError(f"record_format must be Format.JSON or Format.proto(...); got {type(fmt).__name__}")


def _options_for_record_type(
    options: Optional["_core.StreamConfigurationOptions"], record_type
) -> "_core.StreamConfigurationOptions":
    """Build a `StreamConfigurationOptions` with `record_type` applied without
    mutating the caller's instance.

    If the caller did not pass `options`, return a fresh default with
    `record_type` set. Otherwise, construct a new `StreamConfigurationOptions`
    from the caller's field values — the original is never modified.

    Note: we can't use `copy.copy` here because PyO3 0.20 native pyclasses
    don't implement `__copy__` / `__reduce__` and Python's copy machinery
    falls back to pickling, which also fails. Explicit reconstruction via
    the kwargs constructor is the portable path.
    """
    if options is None:
        return _core.StreamConfigurationOptions(record_type=record_type)
    return _core.StreamConfigurationOptions(
        max_inflight_records=options.max_inflight_records,
        recovery=options.recovery,
        recovery_timeout_ms=options.recovery_timeout_ms,
        recovery_backoff_ms=options.recovery_backoff_ms,
        recovery_retries=options.recovery_retries,
        server_lack_of_ack_timeout_ms=options.server_lack_of_ack_timeout_ms,
        flush_timeout_ms=options.flush_timeout_ms,
        record_type=record_type,
        stream_paused_max_wait_time_ms=options.stream_paused_max_wait_time_ms,
        callback_max_wait_time_ms=options.callback_max_wait_time_ms,
        ack_callback=options.ack_callback,
    )


class ZerobusSdk:
    """Synchronous Zerobus SDK handle."""

    def __init__(
        self,
        host: str,
        unity_catalog_url: str,
        *,
        application_name: Optional[str] = None,
    ):
        """
        Args:
            host: Fully-qualified Zerobus endpoint.
            unity_catalog_url: Workspace URL used for OAuth token exchange.
            application_name: Optional caller-supplied identifier appended to
                the HTTP `user-agent` for server-side telemetry. The SDK prefix
                (`zerobus-sdk-py/<version>`) is always emitted. Empty strings
                are treated as unset.
        """
        self._inner = _RustZerobusSdk(host, unity_catalog_url, application_name)

    def create_stream(
        self,
        *,
        table: str,
        auth: Auth,
        record_format: FormatSpec,
        options: Optional["_core.StreamConfigurationOptions"] = None,
    ) -> ZerobusStream:
        """Open a JSON or protobuf gRPC ingestion stream.

        `record_format` is named to avoid shadowing the builtin `format()`.
        """
        client_id, client_secret, headers_provider = _split_auth(auth)
        descriptor_bytes, message_name, record_type = _split_format(record_format)
        effective_options = _options_for_record_type(options, record_type)

        rust_stream = self._inner.create_stream(
            table=table,
            client_id=client_id,
            client_secret=client_secret,
            headers_provider=headers_provider,
            descriptor_bytes=descriptor_bytes,
            descriptor_message_name=message_name,
            options=effective_options,
        )
        return ZerobusStream(rust_stream)

    def recreate_stream(self, old_stream: ZerobusStream) -> ZerobusStream:
        """Recreate a closed gRPC stream with the same configuration."""
        rust_stream = self._inner.recreate_stream(old_stream._inner)
        return ZerobusStream(rust_stream)

    def create_arrow_stream(
        self,
        *,
        table: str,
        schema,
        auth: Auth,
        options=None,
    ) -> ZerobusArrowStream:
        """Open an Arrow Flight ingestion stream.

        **Beta**: Arrow Flight is in Beta — the API is stabilising but may still change before reaching GA.
        """
        from zerobus.sdk.shared.arrow import _serialize_schema

        client_id, client_secret, headers_provider = _split_auth(auth)
        schema_bytes = _serialize_schema(schema)
        rust_stream = self._inner.create_arrow_stream(
            table=table,
            schema_ipc_bytes=schema_bytes,
            client_id=client_id,
            client_secret=client_secret,
            headers_provider=headers_provider,
            options=options,
        )
        return ZerobusArrowStream(rust_stream)

    def recreate_arrow_stream(self, old_stream: ZerobusArrowStream) -> ZerobusArrowStream:
        """Recreate a closed Arrow stream."""
        rust_stream = self._inner.recreate_arrow_stream(old_stream._inner)
        return ZerobusArrowStream(rust_stream)


# Re-export common types for convenience
HeadersProvider = _core.HeadersProvider
RecordType = _core.RecordType
StreamConfigurationOptions = _core.StreamConfigurationOptions
AckCallback = _core.AckCallback
ZerobusException = _core.ZerobusException
NonRetriableException = _core.NonRetriableException

__all__ = [
    "ZerobusSdk",
    "ZerobusStream",
    "ZerobusArrowStream",
    "StreamConfigurationOptions",
    "RecordType",
    "AckCallback",
    "HeadersProvider",
    "ZerobusException",
    "NonRetriableException",
    "OAuth",
    "Headers",
    "Format",
]
